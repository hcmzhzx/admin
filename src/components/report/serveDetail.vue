         ...
         <div class="table-content">
            <table class="table table-bordered table-hover">
               ...
               <tr v-for="item in detailList" :key="item.id">
                  <td>{{item.user.after_admin_id}}</td>
...
<script>
   ...
   export default {
      ...
      data(){
         return {
            text: [{txt:'售后服务报表',src:'report_aftersalecounter'},{txt:'销售详情'}],
            detailList:[],  // 详情数据
            ID:'',  // 员工id
            time:'',  //日期时间
      ...
      created(){
         ...
         this.$http.get(`report/service_user?include=user,partner&admin_id=${id}&begin_time=${time}`).then(res=>{
      ...
      methods:{
         ...
         pageChange(val){
            ...
            this.$http.get(`report/service_user?include=user&admin_id=${this.ID}&begin_time=${this.time}&page=${val}`).then(res=>{
               this.detailList = res.data;
               this.meta = res.meta.pagination;
               loading.close(); // 结束loading
            })
