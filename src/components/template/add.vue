      ...
      <div class="content">
         <form method="post" class="post" @submit.prevent="sendForm">
            <div class="form-group">
               ...
               <input type="text" id="title" class="form-control small" name="title" data-rule="*" data-errmsg="请填写模板标题" data-sync="true">
            </div>
            <div class="form-group">
               <label for="desc">模板描述</label>
               <input type="text" id="desc" class="form-control medium" name="desc" data-rule="*" data-errmsg="请填写模板描述" data-sync="true">
            </div>
            <div class="form-group">
               <label for="dir">模板目录</label>
               <input type="text" id="dir" class="form-control small" name="dir" data-rule="*" data-errmsg="请填写模板目录" data-sync="true">
            </div>
            <div class="form-group">
...
<script>
   ...
   export default {
      ...
      data(){
         return {
            text: [{txt:'模板设置', src:'template_index'}, {txt:'添加模板'}],
            fileList:[],  // file 对象数组
      ...
      methods:{
         ...
         sendForm(e){
            new CheckForm(e,err=>{
               ...
               this.$http.post('theme',Form).then(res=>{
                  loading.close(); // 结束loading
                  this.$router.go(-1);
               }).catch(err=>{
                  console.log(err);
