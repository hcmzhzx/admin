      ...
      <div class="content">
         <form class="post" @submit.prevent="sendForm">
            <div class="form-group">
               ...
               <input type="text" id="title" ref="title" class="form-control small" name="title" data-rule="*" data-errmsg="品牌必须填写" data-sync="true">
            </div>
            <div class="form-group">
      ...
      methods:{
         ...
         sendForm(e){
            new CheckForm(e,err=>{
               ...
               const tit = Object.keys(post).join(''), form = new FormData();
               form.append(tit,post.title.trim());  // 标题
               ...
               if(e.target.querySelector('input[type=hidden]')){
                  form.append('banner',e.target.querySelector('input[type=hidden]').value); // banner图片
               }
               this.$http.post('brands',form).then(res=>{
                  this.$router.push({name:'brand_index'})
               })
            })
         },
         // 更换图片请求
